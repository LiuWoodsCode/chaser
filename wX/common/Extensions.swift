...
extension String {
    ...
    func getHtml() -> String {
        UtilityIO.getStringFromUrl(self)
    }

    func getHtmlWithRetry(_ expectedMinSize: Int) -> String {
        var html = self.getHtml()
        if html.count < expectedMinSize {
            sleep(1)
            html = self.getHtml()
        }
        return html
    ...
    func getHtmlUtf8() -> String {
        UtilityIO.getStringFromUrlUtf8(self)
    }

    func getHtmlWin1252() -> String {
        UtilityIO.getStringFromUrlWin1252(self)
    }
    ...
    func getNwsHtmlWithRetry(_ expectedMinSize: Int) -> String {
        var html = self.getNwsHtml()
        if html.count < expectedMinSize {
            sleep(1)
            html = self.getNwsHtml()
        }
        return html
    }

    func getImage() -> Bitmap {
        UtilityIO.getBitmapFromUrl(self)
    }
...
//    func goToVC(_ target: UIHostingController<some View>) {
//        target.modalPresentationStyle = .fullScreen
//        present(target, animated: UIPreferences.backButtonAnimation, completion: nil)
//        // uiv.pushViewController(swiftUIViewController, animated: true)
//    }
}
