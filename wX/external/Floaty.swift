...
open class Floaty: UIView {
     ...
     */
    @IBInspectable
    open var animationSpeed = 0.1

    /**
     Button color.
     */
    @IBInspectable
    open var buttonColor = UIColor(red: 73/255.0, green: 151/255.0, blue: 241/255.0, alpha: 1)
     ...
     */
    @IBInspectable
    open var plusColor = UIColor(white: 0.2, alpha: 1)

    /**
     Background overlaying color.
     */
    @IBInspectable
    open var overlayColor = UIColor.black.withAlphaComponent(0.3)
     ...
     */
    @IBInspectable
    open var itemButtonColor = UIColor.white

    /**
     Child item's default title label color.
     */
    @IBInspectable
    open var itemTitleColor = UIColor.white

    /**
     Child item's image color
     */
    @IBInspectable
    open var itemImageColor: UIColor?
     ...
     */
    @IBInspectable
    open var itemShadowColor = UIColor.black
     ...
     */
    fileprivate var tintLayer = CAShapeLayer()
     ...
     */
    fileprivate var overlayViewDidCompleteOpenAnimation = true

    /**
     If you created this object from storyboard or `initWithFrame`, this property set true.
     */
    fileprivate var isCustomFrame = false

    /**
     An accessibility button for the main Fab Button
     */
    fileprivate var accessibilityView = UIView()
    ...
    @objc public func open() {
        ...
        if items.count > 0 {
            ...
            UIView.animate(withDuration: 0.3, delay: 0,
                           ...
                           options: UIView.AnimationOptions(), animations: { () in
    ...
    @objc internal func deviceOrientationDidChange(_ notification: Notification) {
        guard let keyboardSize: CGFloat =
            (notification.userInfo?[UIResponder.keyboardFrameBeginUserInfoKey] as? NSValue)?.cgRectValue.size.height else {
                return
        }
    ...
    @objc internal func keyboardWillShow(_ notification: Notification) {
        guard let keyboardSize: CGFloat =
            (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.size.height,
...
extension Floaty {
    ...
    fileprivate func popAnimationWithClose(group: DispatchGroup) {
        ...
        for item in items.reversed() {
            if item.isHidden == true { continue }
            group.enter()
            UIView.animate(withDuration: 0.15, delay: delay, options: [], animations: { () in
                item.layer.transform = CATransform3DMakeScale(0.4, 0.4, 1)
                item.alpha = 0
            }, completion: { _ in
                group.leave()
    ...
    fileprivate func slideLeftAnimationWithClose(group: DispatchGroup) {
        ...
        for item in items.reversed() {
            ...
            group.enter()
            UIView.animate(withDuration: 0.3, delay: delay, options: [], animations: { () in
    ...
    fileprivate func slideUpAnimationWithOpen(group: DispatchGroup) {
        ...
        for item in items {
            ...
            group.enter()
            UIView.animate(withDuration: 0.2, delay: 0, options: [], animations: { () in
                item.frame.origin.y = -itemHeight
                item.alpha = 1
            }, completion: { _ in
                group.leave()
 ...
 */
extension Floaty {
    fileprivate func degreesToRadians(_ degrees: CGFloat) -> CGFloat { degrees / 180.0 * CGFloat.pi }
}
