...
final class ObjectModel {
    ...
    func setModelVars(_ modelName: String) {
        ...
        case "NCEP:HRRR":
            ...
            runs.removeAll()
            (0...22).forEach {
                runs.append(To.stringPadLeftZeros($0, 2) + "Z")
            }
        ...
        case "NCEP:GEFS-WAVE":
            ...
            paramLabels = UtilityModelNcepInterface.labelsGefsWave
            sectors = UtilityModelNcepInterface.sectorsGefsWave
            times.removeAll()
            loadTimeList3(0, 239, 3)
            loadTimeList3(240, 385, 6)
            setupListRunZ()
        case "NCEP:GFS-WAVE":
            ...
            setupListRunZ()
        case "NCEP:STOFS":
            params = UtilityModelNcepInterface.modelStofsParams
            paramLabels = UtilityModelNcepInterface.modelStofsLabels
    ...
    func loadTimeList3(_ from: Int, _ to1: Int, _ by: Int) {
        stride(from: from, to: to1, by: by).forEach {
            times.append(To.stringPadLeftZeros($0, 3))
        }
    }

    func loadTimeList3by2(_ from: Int, _ to1: Int, _ by: Int) {
        stride(from: from, to: to1, by: by).forEach {
            times.append(To.stringPadLeftZeros($0, 2))
        }
    }
