...
final class UtilityModelSpcHrefInputOutput {
    ...
    static func getRunTime() -> RunTimeData {
        ...
        let htmlRunStatus = (GlobalVariables.nwsSPCwebsitePrefix + "/exper/href/").getHtmlUtf8()
        ...
        runData.appendListRun(ObjectDateTime.generateModelRuns(mostRecentRun, 12, "yyyyMMddHH"))
    ...
    static func getImage(_ om: ObjectModel) -> Bitmap {
        ...
        products.forEach { product in
            ...
            if product.contains("cref_members") {
                let paramArr = product.split(" ")
                let infoUrl = GlobalVariables.nwsSPCwebsitePrefix + "/exper/href/graphics/models/href/" + year +
                        "/" + month + "/" + day + "/" + hour + "00/f0" + om.time + "00/" + paramArr[0] + "." + sector.lowercased() + ".f0" + om.time + "00.png"
                urls.append(infoUrl)
            }
            urls.append(url)
        }
        if products.contains("cref_ps") {
            urls.append(GlobalVariables.nwsSPCwebsitePrefix + "/exper/href/graphics/blank_maps/" + sector + ".ps.href.png")
        } else {
            urls.append(GlobalVariables.nwsSPCwebsitePrefix + "/exper/href/graphics/blank_maps/" + sector + ".png")
        }
        let bitmaps = urls.map { Bitmap($0) }
        return UtilityImg.layerDrawableToBitmap(bitmaps)
    }
}
