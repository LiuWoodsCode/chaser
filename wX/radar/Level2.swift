...
final class Level2 {
    ...
    static func decode(_ radarBuffers: MetalRadarBuffers, _ fileStorage: FileStorage, _ days: MemoryBuffer, _ msecs: MemoryBuffer) {
        let velocityProd = (radarBuffers.levelData.productCode == 154)
        let dis2 = fileStorage.memoryBuffer
            ...
            if !velocityProd {
                (0..<numberOfRadials).forEach {
                    if highReflectivity[$0].elevationNum == 1 {
                        radarBuffers.levelData.radialStartAngle.putFloat(450.0 - highReflectivity[$0].azimuth)
                        highReflectivity[$0].readData(dis2, reflectivityHigh, radarBuffers.levelData.binWord)
                    }
                }
            } else {
                ...
                (0..<numberOfRadials).forEach {
                    if highVelocity[$0].elevationNum == 2 {
                        radarBuffers.levelData.radialStartAngle.putFloat(450.0 - highVelocity[$0].azimuth)
                        highVelocity[$0].readData(dis2, velocityHigh, radarBuffers.levelData.binWord)
                    }
                }
            }
        }
