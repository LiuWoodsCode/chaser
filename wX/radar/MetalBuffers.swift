...
class MetalBuffers {
    ...
    func putColor(_ byte: UInt8) {
        metalBuffer.append(Float(byte) / Float(255.0))
    }

    func putColor3(_ color: Int) {
        putColor(Color.red(color))
        putColor(Color.green(color))
        putColor(Color.blue(color))
    }
    ...
    func draw(_ pn: ProjectionNumbers) {
        switch typeEnum {
        case .HI:
            MetalBuffers.redrawTriangleUp(self, pn)
        case .SPOTTER:
            MetalBuffers.redrawCircle(self, pn)
        case .TVS:
            MetalBuffers.redrawTriangleUp(self, pn)
        case .LOCDOT:
            MetalBuffers.redrawCircle(self, pn)
        case .WIND_BARB_CIRCLE:
            MetalBuffers.redrawCircleWithColor(self, pn)
        default:
            MetalBuffers.redrawTriangle(self, pn)
        }
    }

    static func redrawTriangle(_ buffers: MetalBuffers, _ pn: ProjectionNumbers) {
        NexradRenderUtilities.genTriangle(buffers, pn)
    }

    static func redrawTriangleUp(_ buffers: MetalBuffers, _ pn: ProjectionNumbers) {
        NexradRenderUtilities.genTriangleUp(buffers, pn)
    }

    static func redrawCircle(_ buffers: MetalBuffers, _ pn: ProjectionNumbers) {
        NexradRenderUtilities.genCircle(buffers, pn)
    }

    static func redrawCircleWithColor(_ buffers: MetalBuffers, _ pn: ProjectionNumbers) {
        NexradRenderUtilities.genCircleWithColor(buffers, pn)
    }
}
