...
final class NexradLevel3HailIndex {

    private static let pattern = "(\\d+) "

    static func decode(_ projectionNumbers: ProjectionNumbers, _ fileStorage: FileStorage) {        
        let rawData = NexradLevel3TextProduct.download("HI", projectionNumbers.getRadarSite())
        var stormList = [Double]()
        if rawData.count > 10 {
            let position = rawData.parseColumn("AZ/RAN(.*?)V")
            let hailPercent = rawData.parseColumn("POSH/POH(.*?)V")
            let hailSize = rawData.parseColumn("MAX HAIL SIZE(.*?)V")
