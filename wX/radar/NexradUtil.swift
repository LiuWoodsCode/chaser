...
final class NexradUtil {
    ...
    static func isRidTdwr(_ radarSite: String) -> Bool {
        RadarSites.tdwrRadarCodes().contains(radarSite)
    }

    static func getRadarTimeStamp(_ nexradLevelData: NexradLevelData) -> String {
        let radarTimeStamp = nexradLevelData.radarInfo
        var radarTimeFinal = ""
        if radarTimeStamp != "" {
    ...
    static func isRadarTimeOld(_ nexradLevelData: NexradLevelData) -> Bool {
        // 1 min is 60,000 ms
//        print(nexradLevelData.radarAgeMilli)
        if nexradLevelData.radarAgeMilli > 20 * 60000 {
            return true
        }
        return false
    }
