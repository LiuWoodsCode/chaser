...
final class RadarSites {

    private static let sites = Sites(name, lat, lon, true)

    private static func nexradRadarCodes() -> [String] {
        var radars = [String]()
    ...
    static func tdwrRadarCodes() -> [String] {
        var radars = [String]()
        for radar in sites.codeList {
            if radar.count == 4 {
                radars.append(radar)
    ...
    static func nexradRadars() -> [String] {
        var radars = [String]()
        for radar in sites.nameList {
            let tokens = radar.split(":")
            if tokens[0].count == 3 {
                radars.append(radar)
            }
        }
    ...
    static func getName(_ radarSite: String) -> String {
        name[radarSite] ?? ""
    }

    static func getLatLon(_ radarSite: String) -> LatLon {
        let lat = lat[radarSite] ?? ""
        let lon = "-" + (RadarSites.lon[radarSite] ?? "")
        return LatLon(lat, lon)
    }

    static func getNearest(_ location: LatLon, _ cnt: Int, includeTdwr: Bool = true) -> [Site] {
        var radarSites = [Site]()
        var allRadars = nexradRadarCodes()
        if includeTdwr {
            allRadars += tdwrRadarCodes()
        }
        allRadars.forEach {
            radarSites.append(
                Site.fromLatLon($0, getLatLon($0), LatLon.distance(location, getLatLon($0))))
        }
        radarSites.sort { $0.distance < $1.distance }
        return Array(radarSites[0...cnt])
    }

    static func getNearestCode(_ location: LatLon, includeTdwr: Bool = true) -> String {
        getNearest(location, 5, includeTdwr: includeTdwr)[0].codeName
    }
    ...
    private static let name = [
        ...
        "GUA": "GU, Andersen AFB",
//        "PLA": "NA, Lajes Field, Azores", // TODO FIXME
...
//        "ODN": "NA, Kadena Air Base, Japan",
    ...
    private static let lat = [
        ...
        "TADW": "38.695", // Andrews Air Force Base (ADW), MD TADW LWX 346
        ...
        "TICH": "37.506844", // Wichita (ICH), KS TICH ICT 1,350
