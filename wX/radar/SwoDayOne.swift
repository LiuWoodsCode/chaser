...
final class SwoDayOne {
    ...
    static let colors = [
        "HIGH": Color.MAGENTA,
        "MDT": Color.RED,
        "ENH": Color.rgb(255, 140, 0),
        "SLGT": Color.YELLOW,
        "MRGL": Color.rgb(0, 100, 0)
    ]
    ...
    static func get() {
        if timer.isRefreshNeeded() {
            let day = 1
            let html = (GlobalVariables.nwsSPCwebsitePrefix + "/products/outlook/KWNSPTSDY" + String(day) + ".txt").getHtml()
