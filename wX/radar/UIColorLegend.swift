...
final class UIColorLegend: UIView {
    ...
    func drawRect(_ index: Int, _ scaledHeight: CGFloat) {
        context!.addRect(
            CGRect(
                x: widthStarting,
                y: CGFloat(index) * scaledHeight + startHeight,
                width: width + widthStarting,
                height: CGFloat(index) * scaledHeight + scaledHeight + startHeight))
        context!.fillPath()
    }

    func drawText(_ string: String, _ y: CGFloat) {
        let textAttributes = [
        ...
        drawTextString(context!, strn, textAttributes, width + textFromLegend, y)
    }
    ...
    func setColor(_ red: UInt8, _ green: UInt8, _ blue: UInt8) {
        Color.rgbToUIColor(red, green, blue).set()
    }

    func setColorWithBuffers(_ prodId: Int, _ index: Int) {
        setColor(
            ColorPalette.colorMap[prodId]!.redValues.get(index),
            ColorPalette.colorMap[prodId]!.greenValues.get(index),
            ColorPalette.colorMap[prodId]!.blueValues.get(index)
        )
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        case "DSP":
            ...
            while j > 0 {
                let yVar1 = CGFloat(255.0 / NexradUtil.wxoglDspLegendMax)
                    * scaledHeightVel
                    * CGFloat(NexradUtil.wxoglDspLegendMax - j)
                drawText(
                    String(j).truncate(4) + units,
                    yVar1 + heightFudge + startHeight
                )
                if !unitsDrawn {
                    unitsDrawn = true
                    units = ""
