...
final class VcSettingsLocationEdit: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    private var toolbarBottom = Toolbar()
    ...
    func setupToolbar() {
        ...
        let saveButton = ToolbarIcon(self, .save, #selector(saveClicked), forceLight: true)
        let searchButton = ToolbarIcon(self, .search, #selector(searchClicked), forceLight: true)
        let deleteButton = ToolbarIcon(self, .delete, #selector(deleteClicked))
        let gpsButton = ToolbarIcon(self, .gps, #selector(gpsClicked), forceLight: true)
        var itemsBottom = [doneButton2, GlobalVariables.flexBarButton, helpButton]
        ...
        toolbarBottom.setConfigWithUiv(self)
    ...
    func setupUI() {
        ...
        statusTextView = Text("")
        let textViews = [labelTextView, latTextView, lonTextView, statusTextView]
        ...
        let vbox = VBox(textViews)
        vbox.addWidget(mapView)
        vbox.noAutoresizingIntoConstraints()
        view.addSubview(vbox.get())
        var bottomSpace: CGFloat
        if #available(iOS 26, *) {
            bottomSpace = UtilityUI.getBottomPadding()
        } else {
            bottomSpace = UIPreferences.toolbarHeight + UtilityUI.getBottomPadding()
        }
        let topSpace = UIPreferences.toolbarHeight + UtilityUI.getTopPadding()
        vbox.get().leadingAnchor.constraint(equalTo: view.leadingAnchor).isActive = true
        vbox.get().trailingAnchor.constraint(equalTo: view.trailingAnchor).isActive = true
        vbox.get().topAnchor.constraint(equalTo: view.topAnchor, constant: topSpace).isActive = true
        vbox.get().bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -1 * bottomSpace).isActive = true
        view.bringSubviewToFront(toolbarBottom)
    }
    ...
    func setupInitialMap() {
        ...
        if settingsLocationEditNum == "0" {
            ...
            Map.centerMapOnLocationEdit(mapView, location: locationC, regionRadius: 50000.0)
        }
    }
    ...
    @objc func saveClicked() {
        ...
        var latString = latTextView.text
        var lonString = lonTextView.text
        if latTextView.text.contains("CANADA:") && lonTextView.text != "" {
            ...
            if latTextView.text.split(":").count > 2 {
                latString = latTextView.text.split(":")[2]
            }
            if lonTextView.text.contains(":") {
                lonString = "-" + lonTextView.text.split(":")[1]
            }
        }
    ...
    func saveFromMap(_ locationName: String, _ lat: String, _ lon: String) {
        ...
        let status = Location.save(numLocsLocalStr, LatLon(lat, lon), labelTextView.text)
        latTextView.text = lat
        lonTextView.text = lon
        statusTextView.text = status
        view.endEditing(true)
