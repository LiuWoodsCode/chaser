...
final class UtilitySpcMesoInputOutput {
    ...
    static func getImage(_ product: String, _ sector: String) -> Bitmap {
        ...
        var showRadar = Utility.readPref(prefModel + "_SHOW_RADAR", "false").hasPrefix("t")
        ...
        if product == "hodo" || product.hasPrefix("skewt") {
            showRadar = true
        }
    ...
    static func getAnimation(_ sector: String, _ product: String, _ frameCount: Int) -> AnimationDrawable {
        var urls = [String]()
        let html = (GlobalVariables.nwsSPCwebsitePrefix + "/exper/mesoanalysis/new/archiveviewer.php?sector=19&parm=pmsl").getHtmlUtf8()
        ...
        if timeList.count > frameCount {
            stride(from: (frameCount - 1), to: -1, by: -1).forEach { index in
                urls.append(GlobalVariables.nwsSPCwebsitePrefix + "/exper/mesoanalysis/s" + sector + "/" + product + "/" + product + "_" + timeList[index] + ".gif")
            }
        }
        return UtilityImgAnim.getAnimationDrawableFromUrlList(urls, addWhiteBg: true)
    }
}
