...
final class VcLsrByWfo: UIwXViewController, MKMapViewDelegate {
    ...
    func getLsrFromWfo(_ wfo: String) {
        ...
        if numberLSR == "" {
            ...
            stride(from: 1, to: maxVers, by: 2).forEach { version in
                _ = FutureVoid({ self.wfoProd.append(DownloadText.getTextProductWithVersion("LSR" + wfo, version)) }, display)
            }
        }
    }
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        coordinator.animate(alongsideTransition: nil) { _ in
            self.map.setupLocations(WfoSites.sites.codeList)
            self.display()
        }
    }
}
