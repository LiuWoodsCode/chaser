...
final class NumberPicker {
    ...
    init(_ uiv: UIwXViewController, _ prefVar: String) {
        if UtilityUI.isTablet() {
            ...
            suffix = "_C"
        #endif
        label = NumberPicker.prefToLabel[prefVar]!
        initialValue = Utility.readPrefInt(prefVar, NumberPicker.prefToInitialValue[prefVar + suffix]!)
        self.prefVar = prefVar
        slider = UISlider()
        slider.minimumValue = NumberPicker.prefToMin[prefVar]!
        slider.maximumValue = NumberPicker.prefToMax[prefVar]!
    ...
    private func setLabel() {
        ...
        button.setTitle(
            label + "(" + String(NumberPicker.prefToInitialValue[prefVar + suffix]!) + "): " + String(Int(slider.value)) + " ",
            for: .normal
        )
        button.titleLabel?.font = FontSize.medium.size
    }
    ...
    @objc func sliderValueDidChange(_ sender: UISlider!) {
        setLabel()
        Utility.writePrefInt(prefVar, Int(sender!.value))
    }
    ...
    private static let prefToMax: [String: Float] = [
        "RADAR_LOCDOT_SIZE": 14.0,
        "RADAR_SPOTTER_SIZE": 14.0,
        "RADAR_HI_SIZE": 14.0,
        "RADAR_TVS_SIZE": 14.0,
        "RADAR_AVIATION_SIZE": 14.0,
