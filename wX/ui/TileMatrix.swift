...
final class TileMatrix {
    ...
    convenience init(_ uiv: UIViewController, _ boxMain: HBox, _ tabType: TabType) {
        self.init()
        self.uiv = uiv
        ...
        var jIndex = 0
        while true {
            let hbox = HBox(.fill, UIPreferences.stackviewCardSpacing)
            var index = 0
            for _ in (0...(rowCount - 1)) {
                if jIndex >= icons.count {
                    _ = TileImage(hbox, iconsPerRow)
                } else {
                    let tile = TileImage(hbox, icons[jIndex], jIndex, iconsPerRow, labels[jIndex])
                    switch tabType {
                    case .spc:
                        tile.connect(GestureData(self, #selector(imgClickedSpc)))
                    case .misc:
                        tile.connect(GestureData(self, #selector(imgClickedMisc)))
                    }
                }
                index += 1
                jIndex += 1
    ...
    @objc func imgClicked(sender: GestureData) {
        switch tabType {
        case .spc:
    ...
    @objc func imgClickedSpc(sender: GestureData) {
    ...
    @objc func imgClickedMisc(sender: GestureData) {
        let iconTitle = icons[sender.view!.tag]
        switch iconTitle {
        case "ncep":
    ...
    @objc func menuClicked() {
        Route.subMenuClicked(uiv!, menuButton)
    }
