...
final class UtilityMath {
    ...
    static func computeTipPoint(_ x0: Double, _ y0: Double, _ x1: Double, _ y1: Double, _ right: Bool) -> [Double] {
        ...
        let dirX = dx / length
        let dirY = dy / length
        let height = sqrt(3.0) / 2.0 * length
        let cx = x0 + dx * 0.5
        let cy = y0 + dy * 0.5
        let pDirX = -dirY
        let pDirY = dirX
    ...
    static func fahrenheitToCelsius(_ value: Double) -> String {
        String(Int(round((value - 32.0) * 5.0 / 9.0)))
    }

    static func celsiusToFahrenheit(_ value: String) -> String {
        String(round((To.double(value)) * 9.0 / 5.0 + 32.0))
    }
    ...
    static func knotsToMph(_ value: String) -> String {
        To.string(Int(round(To.double(value) * 1.151)))
    }
    ...
    static func pixPerDegreeLon(_ centerX: Double, _ factor: Double) -> Double {
        let radius = (180.0 / Double.pi) * (1.0 / cos(deg2rad(30.51))) * factor
        return radius * (Double.pi / 180.0) * cos(deg2rad(centerX))
    }
    ...
    static func bearingToDirection(_ direction: Int) -> String {
        let windDirections = ["N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE", "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW", "N"]
        let normalizedDirection = direction % 360
