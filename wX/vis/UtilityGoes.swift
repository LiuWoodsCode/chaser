...
final class UtilityGoes {
    ...
    static func getImage(_ product: String, _ sector: String) -> Bitmap {
        ...
        var url = GlobalVariables.goes16Url + "/" + satellite + "/ABI/" + sectorLocal + "/" + product + "/" + getImageSize(sector) + ".jpg"
        if product == "GLM" {
    ...
    static func getAnimation(_ product: String, _ sector: String, _ frameCnt: Int) -> AnimationDrawable {
        ...
        switch sector {
        case "FD":
            url = "https://star.nesdis.noaa.gov/GOES/fulldisk_band.php?sat=G19&band=" + product.replace("GLM", "EXTENT3") + "&length=" + frameCount
        case "CONUS", "CONUS-G17":
            url = "https://star.nesdis.noaa.gov/GOES/conus_band.php?sat=" + satellite + "&band=" + product.replace("GLM", "EXTENT3") + "&length=" + frameCount
        default:
            url = "https://star.nesdis.noaa.gov/GOES/sector_band.php?sat=" + satellite + "&sector=" + sector + "&band=" + product.replace("GLM", "EXTENT3") + "&length=" + frameCount
        }
        let html = url.getHtmlUtf8().replaceAll("\n", "").replaceAll("\r", "")
        let imageHtml = html.parse("animationImages = \\[(.*?)\\];")
        let imageUrls = imageHtml.parseColumn("'(https.*?jpg)'")
        return UtilityImgAnim.getAnimationDrawableFromUrlList(imageUrls)
    }
    ...
    static let sectorToLatLon = [
        "cgl": LatLon(39.123405, -82.532938),  // wellston, Oh
        "ne": LatLon(39.360611, -74.431877),   // Atlantic City, NJ
        "umv": LatLon(40.622777, -93.934116),  // Lamoni, IA
        "pnw": LatLon(41.589703, -119.858865), // Vya, NV
        "psw": LatLon(38.524448, -118.623611), // Hawthorne, NV
        "nr": LatLon(41.139980, -104.820244),  // Cheyenne, Wy
        "sr": LatLon(34.653376, -108.677852),  // Fence Lake, NM
        "sp": LatLon(31.463787, -96.058022),   // Buffalo, TX
        "smv": LatLon(31.326460, -89.289658),  // Hattiesburg, MS
        "se": LatLon(30.332184, -81.655647),   // Jacksonville, FL
        "pr": LatLon(18.22919, -66.03470),     // Caguas, PR
        "hi": LatLon(20.764, -155.333),        // Hawaii
        "tpw": LatLon(7.074, -129.370)         // Tropical Pacific (best for American Samoa)
    ]
}
